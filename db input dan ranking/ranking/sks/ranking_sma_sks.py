...
def loadPeserta() :
        sql = '''
        select NO_UJIAN, NAMA, DOMISILI, ASAL_SEKOLAH, TMP, TPA, PP, NA, PILIH1, PILIH2, WAKTU_DAFTAR 
        from data_tes_sma 
        order by NA desc, TMP desc, TPA desc, PP desc        
        '''
        cur = db_ranking.cursor()
        cur.execute(sql)
...
def AmbilNamaTabel():
    db_view = MySQLdb.connect("localhost", "root", "", "viewsda2014")
    cur = db_view.cursor()
